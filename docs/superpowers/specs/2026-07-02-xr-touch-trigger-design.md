...
│ D8 │ 크기 조절              │ 잠금+편집 모드: ☰ 핸들 long-press로 토글,    │
│    │ (2026-07-03)          │ 편집 중에만 모서리 grip 리사이즈,            │
│    │                       │ 종료 시 `[window]` config에 크기 저장        │
│ D9 │ 외형 설정              │ 배경/버튼 투명도 + 색상 테마(accent·tint)를  │
│    │ (2026-07-03)          │ 설정 창에서 조절. 설정 창 자체는 불투명 고정 │
│ D10│ 설정 창 UX            │ 상단 드래그 바 이동. 버튼 3개: 적용(저장)·   │
│    │ (2026-07-03)          │ 뒤로가기(미적용 변경 폐기+닫기)·프로그램     │
│    │                       │ 종료. 외형·크기 변경은 실시간 preview,       │
│    │                       │ 적용 전까지 비영속                           │
│ D11│ 팔레트 레이아웃        │ 클러스터(☰+⚙+상태점, 순서 이대로) 방향은     │
│    │ (2026-07-03)          │ 버튼 흐름과 수직(동적). 기본=세로 모드(창     │
│    │                       │ 240×400·버튼 1열·클러스터 상단). 버튼 그리드  │
│    │                       │ (가로·세로 체크+수). 폰트 버튼크기 비례        │
│    │                       │ (container query). SE grip만. 편집모드 진입    │
│    │                       │ =long-press·해제=짧은 탭. ⚙는 ☰ 크기          │
│ D12│ 마지막 누름 강조       │ 설정 on/off. 켜면 최근 누른 버튼 폰트를 살짝  │
│    │ (2026-07-03)          │ 굵게 + 밑줄(색·투명도 설정). 런타임·비영속    │
│ D13│ active-only heartbeat  │ ping은 active 대상에만(비활성엔 안 보냄).     │
│    │ (2026-07-03)          │ 설정 창에 active 장비 연결 상태 표시(위젯과   │
│    │                       │ 동일). 기존 "모든 등록 대상 ping" 대체        │
└────┴───────────────────────┴─────────────────────────────────────────────┘
```
...
- 상시 모서리 드래그 리사이즈 기각(2026-07-03) — 생방송 중 오터치로 창 변형 위험 + frameless 창 모서리는 터치 타겟이 작아 정밀도 문제. 잠금+편집 모드(D8) 채택.
...
│              │                │ (graphic_id)         │                         │
│ ping         │ 위젯 → UE      │ /xrt/ping (인자 없음)│ 1초 주기, active 대상에만│
│              │ (:8000)        │                      │ (D13 2026-07-03 —       │
│              │                │                      │ 비활성엔 heartbeat 안 감;│
│              │                │                      │ 상태 표시도 active만)    │
...
[appearance]                    # D9 (2026-07-03) — 전 필드 serde default (기존 TOML 호환)
bg_opacity = 0.55               # 팔레트 배경 투명도 0.0~1.0 (--glass-bg 알파)
button_opacity = 0.07           # 버튼 fill 투명도 0.0~1.0
accent = "#4da3ff"              # 테마 accent 색
bg_tint = "#141820"             # 배경 tint 색 (--glass-bg RGB)
highlight_last = false          # D12 (2026-07-03) — 마지막 누른 버튼 폰트 강조 on/off
highlight_color = "#4da3ff"     # D12 (2026-07-03) — 마지막 누름 밑줄 색
highlight_opacity = 1.0         # D12 (2026-07-03) — 밑줄 투명도 0.0~1.0

[window]                        # D8 (2026-07-03) — 편집 모드에서 조절된 크기 저장
width = 240                      # 기본 세로 모드에 맞춘 tall-narrow (D11 2026-07-03)
height = 400

[layout]                        # D11 (2026-07-03) — 버튼 그리드 배치. 전 필드 serde default
horizontal = false              # 가로 배치 체크박스 (기본 세로이므로 false)
vertical = true                 # 세로 배치 체크박스 (기본값 — 버튼 세로 1열)
cols = 3                        # 가로 수(열) — 둘 다 체크일 때만 유효
rows = 2                        # 세로 수(행) — 둘 다 체크일 때 최소 행 수(초과 버튼은 행 확장)
```

## 7. UI/UX

### 팔레트 (메인 창)
- always-on-top, frameless, 창 전체를 glass가 채움.
- 구성 (D11): **컨트롤 클러스터**(햄버거+설정+상태등 한 세트) + **버튼 그리드**(나머지 영역).
  - **컨트롤 클러스터** = `[☰ 핸들][⚙ 설정][●●○ 상태점]` 세 개 묶음(이 순서). 트리거 버튼과 분리. **방향은 버튼 흐름과 수직**(perpendicular, 동적): 세로 버튼 모드 → 클러스터 **가로·상단**, 가로 버튼 모드 → 클러스터 **세로·좌측**. **기본이 세로 모드**이므로 기본 배치 = 클러스터 가로 상단 + 버튼 세로 1열(창 240×400). 그리드가 어떻게 배치돼도 컨트롤 위치가 안정적.
  - **☰ 핸들**(햄버거): 드래그로 팔레트 이동. **long-press(~600ms)로 편집 모드 토글** (D8).
  - **상태점**: 등록 장비당 점 하나 — 활성=채운 점, 비활성=빈 점, 끊김=빨강. "지금 트리거가 어디로 가는지"가 항상 보이는 것이 활성 스위칭 모델의 모드 실수 방지책.
  - **⚙**: 설정 창 열기.
- **버튼 그리드 (D11)**: `config.layout`으로 배치 결정.
  - 가로만 체크 → 1행에 버튼 전부(1×N). 세로만 체크 → 1열에 전부(N×1). 숫자는 단일 축일 때 무시.
  - 둘 다 체크 → `cols`(가로수)×`rows`(세로수) 그리드, 좌상단부터 **행 우선** 채움. 버튼이 슬롯보다 많으면 **열 수(cols) 유지 + 행 수 확장**(모든 버튼 표시). 둘 다 해제 → fallback 1행.
  - **폰트 스케일링 (D11)**: 버튼 글자 크기가 버튼 셀 크기에 비례(CSS container query 단위). 창/버튼이 작아지면 글자도 같이 작아져 넘치지 않음.
  - **마지막 누름 강조 (D12)**: `appearance.highlight_last`가 true면 최근 누른 버튼의 폰트를 살짝 굵게 유지(다음 누름까지). 런타임 상태 — config엔 on/off만, 재시작 시 초기화.
- **편집 모드 (D8)**: 창은 **항상** OS 수준 `resizable = false`(잠금). 편집 모드에서만 **우하단(SE) grip 하나** 노출(D11 — 다른 3개 모서리는 앵커 어긋남으로 제거, 위치 조절은 ☰ 핸들로) + accent 테두리로 모드 상태를 상시 표시(상태점과 같은 모드-실수 방지 철학). grip 드래그는 **수동 delta → `setSize`**(좌상단 고정 앵커. 프로그램적 리사이즈 — 네이티브 `startResizeDragging`은 macOS undecorated 창에서 동작하지 않아 기각, 2026-07-03 실기기 검증). 종료 시 크기를 `[window]` config에 저장, 재기동 시 적용. 방송 중 오터치로 창이 변형되는 사고를 막는 것이 잠금의 목적.
- 창 크기 = 기본 240×400 tall-narrow (세로 모드 — 버튼 1열 + 클러스터 상단), 편집 모드로 사용자 조절 가능. 작은 창이라 아래 데모 페이지의 터치를 가리지 않음(팔레트 창 구조 선택의 핵심 근거).
- 접기(collapse) 기능은 v1 보류.

### 설정 창
- ⚙ 터치 시 별도 창으로 열림. **불투명 고정** — 가독성 우선, 투명도 설정 대상 아님 (D9; glass 창에서 변경, 2026-07-03).
- **상단 드래그 바**로 창 이동 (D10). 입력 요소·버튼은 드래그 영역이 아님.
- 섹션 1 — XR 장비: 추가/삭제, 이름, IP, 활성 checkbox. **active 장비는 연결 상태점 표시**(위젯과 동일: 연결=초록, 끊김=빨강, Unknown=회색; D13). 비활성 장비는 상태점 없음(ping 안 감).
- 섹션 2 — 버튼: 추가/삭제, 라벨, graphic_id, 순서 변경.
- 섹션 3 — 외형 (D9): 배경 투명도·버튼 투명도 슬라이더 + accent·배경 tint 색상 선택 + 팔레트 창 크기(width/height 숫자 입력 — D8 편집 모드와 공존).
- **preview/적용/폐기 모델 (D10)**: 외형·크기 변경은 조작 즉시 팔레트에 실시간 preview(비영속). **[적용]** = config 저장(재기동 후에도 유지) · **[뒤로가기]** = 미적용 변경 폐기(팔레트를 마지막 저장값으로 복원)하고 창 닫기 · **[프로그램 종료]** = 앱 전체 종료. 팔레트에는 종료 경로가 없다 — 오터치 방지상 종료는 설정 창 안쪽에만 (D10).
- 장비/버튼 목록 변경은 preview 대상이 아니며 [적용] 시에만 반영.
...
- 팔레트 접기(collapse).
- 팔레트 창 위치 기억(재기동 시 복원) — 크기는 D8로 저장하나 위치는 v1 보류.
- 부팅 자동 시작.
