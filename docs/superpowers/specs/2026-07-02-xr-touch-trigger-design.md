...
│ D14│ 버튼 = 편집형 OSC 메시지│ 고정 /xrt/graphic+graphic_id 대신 버튼마다    │
│    │ (2026-07-08)          │ 주소 + 타입값 1개(string/int/float/bool/none) │
│    │                       │ 직접 편집. 범용 OSC 컨트롤 서피스. ping/pong  │
│    │                       │ 은 앱 고유 프로토콜로 고정(편집 대상 아님)     │
└────┴───────────────────────┴─────────────────────────────────────────────┘
```
...
│ 메시지       │ 방향           │ 주소 + 인자          │ 규칙                    │
├──────────────┼────────────────┼──────────────────────┼─────────────────────────┤
│ 트리거       │ 위젯 → UE      │ 버튼별 편집 주소     │ 단발 전송, 활성 대상에만 │
│ (D14)        │ (:8000)        │ arg0: 타입값 1개     │ 반복 전송 금지. 주소는   │
│              │                │ (s/i/f/T·F/없음)     │ "/"시작·공백 없음 검증   │
...
[[buttons]]                     # D14 (2026-07-08) — 버튼마다 OSC 주소 + 타입값 편집
label = "그래픽 A"              # 팔레트 표시 라벨
address = "/xrt/graphic"        # OSC 주소 (검증: "/"로 시작·공백 없음)
value = "lower_third_a"         # 값 (문자열 표현; value_type에 따라 파싱)
value_type = "string"           # string | int | float | bool | none(인자 없음)
...
- 구성 (D11): **컨트롤 클러스터**(햄버거+설정+상태등 한 세트) + **버튼 그리드**(나머지 영역).
  ...
  - **☰ 핸들**(햄버거): 드래그로 팔레트 이동. **long-press(1000ms)로 편집 모드 진입 · 편집 중 짧은 탭으로 해제** (D8; 진입 시간은 방송 중 오터치 방지 위해 600→1000ms 상향, 2026-07-03). 창이 움직이면(onMoved) 진입 타이머 즉시 취소.
...
- 섹션 2 — 버튼: 추가/삭제, 라벨, **OSC 주소·값·값타입**(D14), 순서 변경. 주소 검증("/"시작·공백 없음), int/float/bool 타입은 값이 파싱 가능해야 [적용] 통과.
