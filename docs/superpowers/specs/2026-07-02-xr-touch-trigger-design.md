...
- 구성: `[☰ 핸들][●●○ 상태점][버튼들…][⚙]`
  ...
  - **⚙**: 설정 창 열기.
- **편집 모드 (D8)**: 평소엔 크기 잠금(`resizable = false`). 편집 모드에서만 모서리 resize grip 노출 + accent 테두리로 모드 상태를 상시 표시(상태점과 같은 모드-실수 방지 철학). 종료 시 크기를 `[window]` config에 저장, 재기동 시 적용. 방송 중 오터치로 창이 변형되는 사고를 막는 것이 잠금의 목적.
...
- 섹션 3 — 외형 (D9): 배경 투명도·버튼 투명도 슬라이더 + accent·배경 tint 색상 선택 → 팔레트에 즉시 반영.
