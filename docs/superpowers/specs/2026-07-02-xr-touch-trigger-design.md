...
- 구성: `[☰ 핸들][●●○ 상태점][버튼들…][⚙]`
  ...
  - **⚙**: 설정 창 열기.
- **편집 모드 (D8)**: 창은 **항상** OS 수준 `resizable = false`(잠금). 편집 모드에서만 모서리 resize grip 노출 + accent 테두리로 모드 상태를 상시 표시(상태점과 같은 모드-실수 방지 철학). grip 드래그는 **수동 delta → `setSize`**(프로그램적 리사이즈 — 네이티브 `startResizeDragging`은 macOS undecorated 창에서 동작하지 않아 기각, 2026-07-03 실기기 검증). 종료 시 크기를 `[window]` config에 저장, 재기동 시 적용. 방송 중 오터치로 창이 변형되는 사고를 막는 것이 잠금의 목적.
- 창 크기 = 기본은 콘텐츠 크기 720×96 (아래 데모 페이지의 터치를 가리지 않음 — 팔레트 창 구조 선택의 핵심 근거), 편집 모드로 사용자 조절 가능.
